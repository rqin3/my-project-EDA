...
filtered_dta=data %>% 
  select(Metro_Status = GTMETSTA, 
         ...
         Household_Income_Continuous = HWSVAL, 
...
#Changing State Codes to State Abbreviation using maps package
...
#Getting the number of counts of each state and putting it in new column
for (i in 1:nrow(filtered_dta)) {
  x=filtered_dta$State_Abbreviation[i]
  number=length(which(filtered_dta$State_Abbreviation==x))
  filtered_dta$Number_In_State[i]=number
}
...
cleaned_dta$Binary_Household_Income=ifelse(cleaned_dta$Household_Income_Categorical<=14, 1, 2)
cleaned_dta$Binary_Household_Income=as.factor(cleaned_dta$Binary_Household_Income)
```
...
```{r}
#Table with State and Number and Proportion
state_dta = filtered_dta %>% 
...
```{r}
#Everything 14 and below=1, Everything 14 and above=2
cleaned_dta$Binary_Household_Income=ifelse(cleaned_dta$Household_Income_Categorical<=14, 1, 2)
```
